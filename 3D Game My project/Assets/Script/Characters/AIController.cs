...
public class AIController : MonoBehaviour
{
    ...
    //[SerializeField] public GameObject aquiredTargetGameObject;
   ...
   /* private void Start()
    {
        ...
        agent = live_charStats.currentNavMeshAgent;
        live_charStats.navMeAge_spawnPoint=transform.position;
        live_charStats.fov_coneCollider=gameObject.AddComponent(typeof(Collider)) as Collider;
    }*/

    /*private void Awake()
    {
        //playerTransform = GameObject.Find("Player_Charmander").transform;
    ...
    private void Update()
    {
        ...
        if (!live_charStats.playerInputEnable && GetComponent<NavMeshAgent>().enabled && live_charStats.isPlayer)
        {
            ...
            if (agent.remainingDistance <= 1f) { live_charStats.navMeAge_walkPoint = transform.position; }
    ...
    private void Patrolling()
    {
        ...
        if (agent.remainingDistance < 1f) live_charStats.navMeAge_walkPointSet = false;
    ...
    private IEnumerator AI_SpellRoutine()   //Routine AI_Castowanie Spelli
    {
        ...
        else
        {
            ...
            live_charStats.inputCasting = false;
            if (live_charStats.fov_aquiredTargetGameObject != null && live_charStats.gameObject.GetComponent<NavMeshAgent>().enabled == true) agent.SetDestination(live_charStats.fov_aquiredTargetGameObject.transform.position); //ustawia target tak ¿eby nie biegaæ na Patrolling
        }
    }
...
{
  foreach (Collider collider in Physics.OverlapSphere(transform.position, live_charStats.navMeAge_sightRange)) //dla ka¿dego collidera w zasiêgu wzroku

  {
      if (enemiesArray.Contains(collider.tag))                                        //jeœli ma tag zawarty w arrayu enemiesArray
      {
          ...
          live_charStats.navMeAge_targetInSightRange = true;
      ...
      else
      {
          ...
          live_charStats.navMeAge_targetInSightRange = false;
      }
  }
}*/
