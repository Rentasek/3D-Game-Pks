...
public class AIController : MonoBehaviour
{
    ...
    private void AIControllerCheck()
    {
        ...
        if (!live_charStats.charInfo.playerInputEnable && GetComponent<NavMeshAgent>().enabled && live_charStats.charInfo.isPlayer)
        {
            ...
            if (live_charStats.currentCharStatus.isAttacking)
            {
                ...
                if (live_charStats.fov.fov_aquiredTargetGameObject != null && !live_charStats.characterInput.inputMouseCurrentMoving) //przy wciskaniu przucisku porusazania siê nie lockuje targetu
                {
                    transform.LookAt(live_charStats.fov.fov_aquiredTargetGameObject.transform);//Enemy jest zwrócony w stronê Playera
            ...
            if (live_charStats.fov.fov_targetInDynamicSightRange && !live_charStats.fov.fov_targetInAttackRange) Chasing(); // player bez inputa œciga tylko przy close sight range
        ...
        if (live_charStats.currentCharStatus.isDead) { StopMovementNavMeshAgent(); }  //Stop Nav Mesh Agent przy œmierci
    ...
    {
        foreach (Collider collider in Physics.OverlapSphere(transform.position, live_charStats.fov.fov_CurrentDynamicSightRadius)) //dla ka¿dego collidera w zasiêgu wzroku
        {
    ...
    {
        foreach (Collider collider in Physics.OverlapSphere(transform.position, live_charStats.fov.fov_attackRange)) //dla ka¿dego collidera w zasiêgu wzroku
        {
            if (enemiesArray.Contains(collider.tag))                                         //jeœli ma tag zawarty w arrayu enemiesArray
            {                                                      
    ...
    {
        foreach (Collider collider in Physics.OverlapSphere(transform.position, live_charStats.charSkillCombat.spell_MaxRadius * live_charStats.charSkillCombat.spell_AISpellRangeFromMax)) //dla ka¿dego collidera w zasiêgu spella
        {
            if (enemiesArray.Contains(collider.tag))                                         //jeœli ma tag zawarty w arrayu enemiesArray
            {                
    ...
    {
        if (!Physics.Raycast(live_charStats.gameObject.transform.position, live_charStats.gameObject.transform.forward, live_charStats.charSkillCombat.spell_MaxRadius * live_charStats.charSkillCombat.spell_AISpellRangeFromMax, live_charStats.fov.fov_obstaclesLayerMask)) //raycast ¿eby nie bi³ przez œciany
        {
    ...
    {
        live_charStats.navMeshAge.navMeAge_walkPoint = transform.position; //debugging ¿eby nie próbowa³ uciekaæ
