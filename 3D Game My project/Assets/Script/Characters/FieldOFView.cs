...
public class FieldOFView : MonoBehaviour
{
    ...
    /* private void OnValidate()
   {
    ...
    private void FieldOfViewCheck()
    {
        ...
        foreach (Collider collider in Physics.OverlapSphere(live_charStats.gameObject.transform.position, live_charStats.fov._currentDynamicSightRadius)) //dla ka¿dego collidera w zasiêgu wzroku
        {
            if (live_charStats.charInfo._enemiesArray.Contains(collider.tag))    //jeœli ma tag zawarty w arrayu enemiesArray
            {
        ...
        if (live_charStats.fov._targetInDynamicSightRange)
        {
            foreach (Collider collider in Physics.OverlapSphere(live_charStats.gameObject.transform.position, live_charStats.fov._currentDynamicSightRadius)) //dla ka¿dego collidera w zasiêgu wzroku
            {
                if (live_charStats.charInfo._enemiesArray.Contains(collider.tag))    //jeœli ma tag zawarty w arrayu enemiesArray
                {
                    ...
                    {
                        if (!Physics.Raycast(live_charStats.gameObject.transform.position, directionToTarget, live_charStats.fov._currentDynamicSightRadius, live_charStats.fov._obstaclesLayerMask))
                        {
